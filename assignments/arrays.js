...
function sortModels() {
  ...
  for (let i = 0; i < inventory.length; i++) {
   carModels = inventory.map(i => i.car_model);
  }
...
function getCarYears() {
  let carYears = [];
  for(let i = 0; i < inventory.length; i++) {
    carYears = inventory.map(i => i.car_year);
  }
...
function getOldCars() {
  let oldCars = [];
  for(let i = 0; i < inventory.length; i++) {
    oldCars = inventory.filter(i => i.car_year < 2000);
  }
...
function fancyCars() {
  let BMWAndAudi = [];
  for(let i = 0; i < inventory.length; i++) {
    BMWAndAudi = inventory.filter(i => i.car_make == "BMW" || i.car_make == "Audi");
  }
